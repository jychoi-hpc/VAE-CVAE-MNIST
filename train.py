    ...
    for epoch in range(args.epochs):
        ...
        for iteration, (x, y) in enumerate(data_loader):
            ...
            if iteration % args.print_every == 0 or iteration == len(data_loader) - 1:
                ...
                original_images, polar_coordinates = dataset[sample_index]
                if args.conditional:
                    x = original_images
                    recon_images, mean, log_var, z = vae(
                        original_images, polar_coordinates
                    )
                ...
                for p in range(len(original_images)):
                    ...
                    if args.conditional:
                        plt.title(
                            "r={:.2f} θ={:.2f}".format(
                                polar_coordinates[p, 0],
                                polar_coordinates[p, 1],
                            ),
                            # color="black",
